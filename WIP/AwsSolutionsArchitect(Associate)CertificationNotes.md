...
* To more easily manage an organisation with multiple accounts, a service called *AWS Control Tower* is available, for free, that acts as a landing page for multi-account management.
  * Can impose rules across the estate of accounts which specify what the accounts must conform to (Known as *Guardrails*).
  * Offers dashboards for high level overviews of account details
...
## Step Functions
* Orchestrates the execution of AWS services as a state machine
* Upon triggering, each step of the state machine will either be an action for a service to perform, or some condition to evaluate
* The workflows are represented as JSON
  * AWS provides a GUI for generating these JSON documents using drag-and-drop
* A single execution of a workflow can run for up to 1 year
* A workflow can hibernate by sending a token out to some other service, the workflow will resume once it recieves this token back
* Lambdas can be included in a workflow and are often used to perform actions that the workflow cannot otherwise natively perform
...
* Some service sends an event into EventBridge, EventBridge looks at its set of rules and determines if there is one that matches the incoming event. If there is, the event will be forwarded to whatever service the rule specifies.
* Rules can be based on the source of the event or on the contents of the event itself
...
* Although EventBridge is tied to a single account and region, it has the capability to send to other EventBridges in other accounts and regions, effectively nullifying any related limitations.
* Supports event replay
  * Events can be saved to S3 for replay later
  * Useful if adding a new consumer at a later date to others so that it can be brought up to speed
